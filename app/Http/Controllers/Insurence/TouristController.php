...
final class TouristController extends BaseInsuranceController
{
    ...
    {
        return view('pages.insurence.tourist.main', ['product' => $this->getProduct()]);
    }
    ...
    public function getPersons(): View|RedirectResponse
    {
        ...
        if (!$applicant) {
            return redirect()->route('tourist.index', ['locale' => getCurrentLocale()]);
        }

        $persons = $this->sess('persons', []);

        return view('pages.insurence.tourist.persons', compact('applicant', 'persons'));
    }
    ...
    public function calculatePremium(Request $request): JsonResponse
    {
        ...
        try {
            $result = $this->calculateTourist((int) $request->input('sum_insured'));
        } catch (ProviderException $e) {
    ...
    public function addPerson(Request $request): RedirectResponse
    {
        ...
        try {
            $calcResult = $this->calculateTourist($sumInsured);
    ...
    public function confirmPersons(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('tourist.getCalculator', ['locale' => getCurrentLocale()]);
    }
    ...
    public function getCalculator(): View|RedirectResponse
    {
        ...
        return view('pages.insurence.tourist.calculator', compact('applicant', 'persons', 'totalSum', 'totalPremium', 'calculation'));
    }
    ...
    public function storeCalculation(Request $request): RedirectResponse
    {
        ...
        return redirect()->route('tourist.getConfirm', ['locale' => getCurrentLocale()]);
    }
    ...
    public function getConfirm(): View|RedirectResponse
    {
        ...
        return view('pages.insurence.tourist.confirm', compact('applicant', 'persons', 'calculation'));
    }
    ...
    public function storeApplication(Request $request): RedirectResponse
    {
        ...
        $apiBody = $this->buildTouristApiBody($applicant, $persons, $calculation);
        ...
        $contractId = $apiResponse['contract_id'] ?? $apiResponse['id'] ?? $apiResponse['UUID'] ?? uniqid('tourist_');
        $paymeUrl   = $apiResponse['payme_url'] ?? null;
        $clickUrl   = $apiResponse['click_url'] ?? null;

        Log::info('Tourist order created', ['contract_id' => $contractId]);

        return $this->createOrderAndRedirect([
            'product_name'             => __('insurance.tourist.product_name'),
            ...
            'insuranceProductName'     => __('insurance.tourist.product_name'),
        ], self::SESSION_KEY);
    }

    // ─── Private: Build API body ──────────────────────────────────────────────

    private function buildTouristApiBody(array $applicant, array $persons, array $calculation): array
    {
