...
@section('content')
    <section class="py-8 bg-gray-50 min-h-screen">
        <div class="container">
            ...
            <x-insurence.page-header icon="bi-person-heart" :title="__('insurance.accident.product_name')" :subtitle="__('insurance.accident.subtitle')" />
            ...
            <div class="row g-4">
                ...
                <div class="col-lg-8">
                            ...
                            <div class="divide-y">
                                @foreach ($persons as $i => $person)
                                    <div class="px-5 py-3 d-flex align-items-center gap-3">
                                        ...
                                        <form method="POST" action="{{ route('accident.removePerson', ['locale' => getCurrentLocale(), 'index' => $i]) }}" onsubmit="return confirm('{{ __('messages.confirm') }}?')">
                                            @csrf
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4">
                        ...
                        <div id="person_card" style="display:none;">
                            <form action="{{ route('accident.addPerson', ['locale' => getCurrentLocale()]) }}"
                    ...
                    <form action="{{ route('accident.storeCalculation', ['locale' => getCurrentLocale()]) }}" method="POST">
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-4">
                            ...
                            <div class="px-5 py-4 border-top d-flex align-items-center justify-content-between" style="background:#f9fafb;">
                                <a href="{{ route('accident.index', ['locale' => getCurrentLocale()]) }}"
                ...
                <div class="col-lg-4">
                    ...
                    @endphp
                    <x-insurence.insurance-sidebar :title="__('insurance.accident.product_name')" :description="__('insurance.accident.subtitle')" :insuranceSum="$totalSum > 0 ? number_format($totalSum, 0, '.', ' ') . ' UZS' : '0 UZS'"
...
@push('scripts')
    <script>
        ...
        document.getElementById('find_btn').addEventListener('click', async function() {
            ...
            try {
                const resp = await fetch(
                    '{{ route('accident.findPerson', ['locale' => getCurrentLocale()]) }}', {
                        method: 'POST',
                        headers: {
                ...
                document.getElementById('display_name').textContent = d.lastname + ' ' + d.firstname + ' ' + (d
                    .middlename || '');
                document.getElementById('display_passport').textContent = d.passport_seria + d.passport_number +
                    ' | PINFL: ' + d.pinfl;
        ...
        const calcUrl = '{{ route('accident.calculatePremium', ['locale' => getCurrentLocale()]) }}';
