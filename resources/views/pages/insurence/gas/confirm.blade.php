    ...
    <div class="flex items-center gap-2 mb-5">
        @foreach([1 => __('messages.applicant'), 2 => __('messages.property_info'), 3 => __('messages.confirm_details')] as $n => $lbl)
            <div class="flex items-center gap-2">
                @if($n < 3)<div class="w-7 h-7 rounded-full flex items-center justify-center text-white" style="font-size:11px;background:#22c55e;"><i class="bi bi-check"></i></div><span class="text-sm font-medium" style="color:#16a34a;">{{ $lbl }}</span>
                @else<div class="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white" style="font-size:11px;font-weight:700;">3</div><span class="text-sm font-semibold text-blue-600">{{ $lbl }}</span>@endif
            </div>
            @if($n < 3)<div class="flex-1" style="height:1px;background:{{ $n < 3 ? '#bbf7d0' : '#e5e7eb' }};"></div>@endif
        @endforeach
    </div>
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                ...
                <div class="row g-2">
                    @foreach([
                        ...
                        [__('messages.property_short_address'), $property['shortAddress']],
            ...
            <div class="d-flex justify-content-between align-items-center">
                <a href="{{ route('gas.getProperty', ['locale' => getCurrentLocale()]) }}"
                ...
                <form action="{{ route('gas.storeApplication', ['locale' => getCurrentLocale()]) }}" method="POST">
                    @csrf
