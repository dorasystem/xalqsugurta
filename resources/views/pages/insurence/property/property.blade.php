...
<div class="container">
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            ...
            <form action="{{ route('property.storeProperty', ['locale' => getCurrentLocale()]) }}" method="POST" id="prop_form">
                @csrf
                    ...
                    <div class="p-5">
                        ...
                        <div class="row g-3 align-items-end mb-3">
                            <div class="col-sm-8">
                                ...
                                <input type="text" id="cad_input" placeholder="17:02:06:01:01:0001"
                            ...
                            <div class="rounded-xl overflow-hidden" style="border:1.5px solid #e5e7eb;background:#fff;">
                                ...
                                <div class="p-4">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-6">
                                            <p class="text-xs text-gray-400 mb-1">{{ __('messages.region') }}</p>
                                            <p class="text-sm font-semibold text-gray-800 mb-0" id="disp_region">{{ $property['region'] ?? '' }}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    ...
                                    <span class="text-base font-bold text-blue-600" id="amt_display">
                                        {{ number_format($calculation['insurance_amount'] ?? 100000000, 0, '.', ' ') }} UZS
                                    </span>
                                </div>
                                <div class="d-flex justify-content-between text-xs text-gray-400 mb-2">
                                    <span>50 000 000</span><span>500 000 000</span>
                                </div>
                                <input type="range" id="amt_slider" min="50000000" max="500000000" step="5000000"
                                    value="{{ $calculation['insurance_amount'] ?? 100000000 }}" class="w-full">
                                <input type="hidden" name="insurance_amount" id="h_insurance_amount"
                                    value="{{ $calculation['insurance_amount'] ?? 100000000 }}">
                            </div>
                            ...
                            <div class="rounded-xl p-4 mb-4 d-flex align-items-center justify-content-between gap-3"
                                ...
                                <div>
                                    <p class="text-xs text-gray-500 font-semibold mb-1 text-uppercase" style="letter-spacing:.5px;">
                                        {{ __('messages.insurance_premium') }} (0.2%)
                                    </p>
                                    <p class="text-2xl font-bold text-gray-800 mb-0" id="prem_display">
                                        {{ number_format(($calculation['insurance_amount'] ?? 100000000) * 0.002, 0, '.', ' ') }} UZS
                                    </p>
                                </div>
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <button type="submit" id="submit_btn"
                        class="d-flex align-items-center gap-2 px-5 py-2 text-white font-semibold rounded-xl text-sm border-0 transition"
...
(function () {
    ...
    function updateSlider() {
        ...
        var prem = Math.round(val * 0.2 / 100);
        hAmt.value = val;
    ...
    cadBtn.addEventListener('click', async function () {
        ...
        try {
            var res = await fetch('{{ route('fetch.cadaster', ['locale' => getCurrentLocale()]) }}', {
                method: 'POST',
                headers: {
            ...
            document.getElementById('disp_region').textContent   = r.region ?? '';
            ...
            document.getElementById('h_cost').value               = r.cost ?? '';
            document.getElementById('h_tip_text').value           = r.tipText ?? '';
            document.getElementById('h_vid_text').value           = r.vidText ?? '';
            document.getElementById('h_tip').value                = r.tip ?? '';
            document.getElementById('h_vid').value                = r.vid ?? '';
            document.getElementById('h_region').value             = r.region ?? '';
            document.getElementById('h_region_id').value          = r.regionId ?? '';
            document.getElementById('h_district_id').value        = r.districtId ?? '';
            document.getElementById('h_district').value           = r.district ?? '';
            document.getElementById('h_building_type').value      = r.buildingType ?? 1;
            document.getElementById('h_cadastr_issue_date').value = r.cadastrIssueDate ?? '';
            document.getElementById('h_street').value             = r.street ?? '';
            document.getElementById('h_dom_num').value            = r.domNum ?? '';
            document.getElementById('h_kvartira').value           = r.kvartiraNum ?? '';
            document.getElementById('h_neighborhood').value       = r.neighborhood ?? '';
