<div id="applicant-info" class="card {{ old('applicant.lastName') ? '' : 'd-none' }}">
    ...
    <div class="card-body">
        <div class="form-check mb-3">
            <input class="form-check-input @error('is_applicant_owner') is-invalid @enderror" type="checkbox"
                id="is-applicant-owner" name="is_applicant_owner" value="on" {{ old('is_applicant_owner') ? 'checked' : '' }}>
    ...
    <div id="applicant-info-display" class="card-footer {{ old('applicant.lastName') ? '' : 'd-none' }}">
        <div class="row">
            ...
            {{-- Applicant infos will be loaded from session (PersonInfoController) --}}
        </div>
        ...
        <div class="row">
            ...
            <x-inputs.input_form :type="'tel'" :class="'col-md-6'" :idFor="'applicant-phone-number'" :name="'applicant[phoneNumber]'"
                :label="'messages.phone_number'" :placeholder="'messages.phone_number_placeholder'" />

            <x-inputs.input_form :type="'email'" :class="'col-md-6'" :idFor="'applicant-email'" :name="'applicant[email]'"
                :label="'messages.email'" :placeholder="'example@example.com'" />
        </div>
    </div>
</div>
